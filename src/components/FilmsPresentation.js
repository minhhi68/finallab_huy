  ...
  return(
    <Grid container>
      <Grid item xs={12} className="film-title" pl={{xs:1 ,sm:7}}>
        <Typography variant="h5" color="black" align="left" mt={14} ml={2} pl={2} style={{borderLeft:'solid'}}>
         ----- Storyteller-----
        </Typography>
      </Grid>
      ...
      {films.map((film) => (
        ...
        <Grid item xs={12} sm={2} pt={5} mr={{sm:3}} className="film-list">
                    <Link to={`Film/${film.id}`} style={{ textDecoration: "none" }}>
          <Card style={{width:'240px', height:'520px', backgroundColor:'black'}}>
            <CardActionArea>
            <Chip label="Full HD" style={{position:"absolute", backgroundColor:"brown", color:"black"}}/>
              <CardMedia
                component="img"
                style={styles.media}
                image={film.img}
